  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-screen overflow-y-auto">
        <div className="relative">
          <img
            src={pg.thumbnailImageUrl || 'https://via.placeholder.com/400'}
            alt={pg.name}
            className="w-full h-64 object-cover rounded-t-xl"
          />
          <button
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <div className="flex items-center text-gray-600 mb-4">
                <MapPin className="w-5 h-5 mr-2" />
                <span>
                  {pg.address?.line1}, {pg.address?.line2 ? pg.address.line2 + ', ' : ''}
              ...
              <div className="flex items-center mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-5 h-5 ${
                      i < (pg.avgRating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                    }`}
                  />
                ))}
                <span className="ml-2 text-gray-600">
                  {pg.avgRating || 0} ({pg.reviewCount || 0} reviews)
                </span>
              </div>
            </div>
            <div className="text-right">
              <div className="text-3xl font-bold text-blue-600 mb-2">₹{pg.basePrice || 0}/mo</div>
              <div className="text-sm text-gray-600">{pg.pgType || 'N/A'}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <p className="text-gray-600 leading-relaxed">
                {pg.shortDescription || 'No description available.'}
              </p>
            </div>
          ...
          <div className="border-t pt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex items-center">
                <User className="w-5 h-5 text-gray-400 mr-3" />
                <div>
                  <div className="font-medium text-gray-900">{pg.owner || 'Owner Name'}</div>
                  <div className="text-sm text-gray-600">Property Owner</div>
                </div>
              </div>
              <div className="flex items-center">
                <Phone className="w-5 h-5 text-gray-400 mr-3" />
                <div>
                  <div className="font-medium text-gray-900">{pg.contactPhone || 'N/A'}</div>
                  <div className="text-sm text-gray-600">Call for inquiries</div>
                </div>
              </div>
              <div className="flex items-center">
                <Mail className="w-5 h-5 text-gray-400 mr-3" />
                <div>
                  <div className="font-medium text-gray-900">{pg.contactEmail || 'N/A'}</div>
                  <div className="text-sm text-gray-600">Send email</div>
                </div>
              </div>
              <div className="flex items-center">
                <Building className="w-5 h-5 text-gray-400 mr-3" />
                <div>
                  <div className="font-medium text-gray-900">{pg.availabilityStatus || 'Unknown'}</div>
                  <div className="text-sm text-gray-600">Availability Status</div>
                </div>
              </div>
            </div>
            ...
            >
              <Heart
                className={`w-5 h-5 inline mr-2 ${
                  favorites.includes(pg.id) ? 'fill-current' : ''
                }`}
              />
              {favorites.includes(pg.id) ? 'Remove from Favorites' : 'Add to Favorites'}
            </button>
